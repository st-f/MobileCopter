package Model
{
	 ...
	 * @author liaocheng
	 */
	public class ExhaustSmoke extends ObjectContainer3D 
	{
		private var particle:ParticlesContainer;
		
		[Embed(source="/assets/yan.png")]
		private var Img:Class;
		
		public function ExhaustSmoke():void 
		{
			...
			var generater:SingleGenerater = new SingleGenerater(sample, 12);
			...
			var action2:ChangeColorByLifeGlobal = new ChangeColorByLifeGlobal(new ColorTransform(0.5, 0.5, 0.5, 0.65), new ColorTransform(0.5, 0.5, 0.5, 0));
			...
			var action4:ScaleByLifeGlobal = new ScaleByLifeGlobal(4,4.7);
		...
		private function initParticleParam(param:ParticleParam):void
		{
			...
			param.duringTime = Math.random() * .51;
			...
			param["VelocityLocal"] = new Vector3D(r1*cos, Math.random() * .1 + 1000, r1*sin);
